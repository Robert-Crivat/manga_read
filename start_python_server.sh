...
# Uccidi eventuali processi esistenti sulla porta 5000
echo "Chiusura processi esistenti sulla porta 5000..."
...
echo "Il server è disponibile su http://localhost:5000"

# Aspetta un momento per verificare che il server sia partito
sleep 2

# Verifica se il server è in esecuzione
if curl -s http://localhost:5000/search_manga?keyword=test > /dev/null; then
